...
{
    if (Device_Open)
        printk(KERN_INFO "Para uso multiple deñ dispositivo\n");
        //return -EBUSY;
...
{
    if(Device_Open > 0) Device_Open--; /* We're now ready for our next caller */
...
static ssize_t device_read(struct file *filp, char *buffer, size_t length, loff_t * offset)
{
    ...
    if (!mutex_trylock(&sem)) {
        return -EBUSY; // Bloquea hasta que el semaforo este disponible
    }
    
    /* If we're at the end of the message, return 0 signifying EOF */
    if (*msg_Ptr == 0) {mutex_unlock(&sem);return 0;}
